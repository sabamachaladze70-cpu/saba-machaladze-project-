...
const Hero: React.FC<HeroProps> = ({ title, slogan, ctaText, onCtaClick }) => {
  return (
    <div className="bg-primary text-white rounded-2xl overflow-hidden shadow-xl mb-12 relative">
      ...
      <div className="relative z-10 px-6 py-16 sm:px-12 sm:py-24 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between">
        <div className="max-w-2xl">
          ...
          <Button variant="primary" onClick={onCtaClick} className="bg-accent text-white hover:bg-yellow-600 font-semibold px-8 py-3 text-lg rounded-full shadow-md border-none">
            {ctaText}
          </Button>
        </div>
        <div className="mt-10 sm:mt-0 hidden md:block">
          {/* Optional illustration/placeholder for the Hero */}
          <div className="w-64 h-64 bg-accent rounded-full opacity-50 flex items-center justify-center animate-pulse">
